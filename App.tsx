...
function App() {
  ...
  const handleGenerate = useCallback(async (forceUseKey?: string) => {
    ...
    // We intentionally removed process.env.API_KEY fallback here.
  ...
  const handleGenerateVideo = useCallback(async (forceUseKey?: string) => {
    ...
    // Video always requires a paid key or valid user key
    let hasInternalKey = false;
    if (window.aistudio) {
        hasInternalKey = await window.aistudio.hasSelectedApiKey();
    }
  ...
  return (
    <>
      <div className="min-h-screen bg-stone-100 font-sans text-stone-800">
        <Header currentMode={modelMode} onModeChange={setModelMode} />
        <main className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-5xl">
          
          <div className="flex flex-col gap-8">
            {/* Top Section: Info & Inputs */}
            ...
            {/* Results Section - Single Column below Input */}
